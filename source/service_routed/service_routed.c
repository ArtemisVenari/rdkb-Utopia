...
static int radv_start(struct serv_routed *sr)
{
     ...
     */
    char sendImmediateRA[10]={0} ;
    int pid = is_daemon_running(ZEBRA_PID_FILE, "zebra");
    if(pid)
    {
        sysevent_get(sr->sefd, sr->setok, "sendImmediateRA", sendImmediateRA, sizeof(sendImmediateRA));
        if(strcmp(sendImmediateRA,"true") == 0){
                sysevent_set(sr->sefd, sr->setok, "sendImmediateRA", "false" , 0);
                kill(pid, SIGUSR2);
        }
        else{
                kill(pid, SIGUSR1);
